...
(function ($) {
   $.Modal=function (modal) {
    ...
    if(len){
        for(;len--;){
            var modal=new $.Modal(t_toggle[len]);
            modal.init(t_toggle[len]);
        }
    }
})(window.JM.component);
...
(function ($) {
    var strategies={
        ...
        'illegalInp':function (value,errorMsg) {   //   ~!@#$%^&*();<>/' --
            if(/([~!@#$%^&*();<>/']+|(--)+)/i.test(value))return errorMsg;
        }
    };
    $.validator=function () {
        this.cache=[];
...
(function ($) {
    ...
    * */
    ...
    moveImgs.prototype={
        ...
        'move':function (propertyName,speedType,target,iSpeed,time) {
            //animation
            var animation=new JM.component.Animation(this.img);
    ...
    * */
    ...
    var fn = $.Slideshow.prototype,
        slideshow = JM.getEle('.slideshow');
    if(slideshow){
        var slideshow_li = slideshow.querySelectorAll('li'),
            slideshow_img = slideshow.querySelectorAll('img'),
            circle_btn = slideshow.querySelectorAll('p>i'),
            oNextBtn = slideshow.querySelectorAll('.next'),
            oPrevBtn = slideshow.querySelectorAll('.prev');
...
(function ($) {
    var createEachQueItem= function (qTitle,cName,flag,type) {
        ...
        if(flag){
            ...
            op_ul_li1_ul_li1_input.placeholder="必填（至少填写一个，单选和多选题的选项最多15个）";
            //add class
            ...
            //add event
            if(op_ul_li1_ul_li1_i)
                JM.addHandler(op_ul_li1_ul_li1_i,'click',function (e) {
                    JM.stopPropagation(JM.getEvent(e));
            ...
            JM.addHandler(op_ul_li2_p_a,'click',function (e) {
                ...
                var formNode = this.parentNode.parentNode.parentNode.parentNode.parentNode, // form
                    formType = formNode.getAttribute('q_type');
                if (formType == 1 || formType == 2) {
                    input.placeholder="必填（至少填写一个，最多15个）";

                    var choices = formNode.querySelectorAll('.option_item_list li'),
                        c_len = choices.length;
                    if (c_len > 14) {
                        alert('最多只能添加15个选项，超过第15个的选项无法保存！');
                        input.disabled = true;
                    }
                } else {
 ...
 */
    var QueType=function (QueObj) {
        ...
        this.deleteBtn=null;//删除每一个问题的删除键
    ...
    QueType.prototype={
        ...
        'setAttr':function () { //设置一些需要用到的自定义属性
            ...
            if(_self.editor){
                ...
                _self.deleteBtn=JM.getEles('.delete_x');
                ...
                for(var i=0,len=_self.editor.length;i<len;i++){
                    ...
                    _self.deleteBtn[i].setAttribute('q_num', _self.editor[i].getAttribute('q_num'));
            ...
            JM.inIterator(_self.qForm,function(i,item){
                ...
                if(_self.qChoice){
                    ...
                    for(var k=0,len=qChoice_inputs.length;k<len;k++)
                        if(qChoice_inputs[k].value.length !=0 && /[^\s]/.test( qChoice_inputs[k].value ) )
                            choices += qChoice_inputs[k].value + '=' + qChoice_inputs[k].value + '='+ '\'\'' + ( k == (len-1) ? '' : '&' );
                }
    ...
    QueResult.prototype={
        ...
        'createEachQue':function (questions) {
            var str='',
                _self=this,
                index=0;
            //create questionnaire structure
            ...
            JM.inIterator(questions,function (i,item) {
                ...
                switch (item.type){
                    case 1: //single choice
                        var s1 = '';
                        for(var j = 0,len = choice.length-1;j<len;j++)  s1 += '<label><input type="radio">' + choice[j].split('=')[0] + '</label>';
                        _self.qes_choice[i].innerHTML=s1;
                        break;
                    case 2: //more choice
                        var s1 = '';
                        for(var j = 0,len = choice.length-1;j<len;j++)  s1 += '<label><input type="checkbox">' + choice[j].split('=')[0] + '</label>';
                        _self.qes_choice[i].innerHTML=s1;
                        break;
                    case 3: //dropdown select
                        var s1 = '',
                            select = _self.qes_choice[i].querySelector('select');
                       if(select){
                           for(var j = 0,len = choice.length-1;j<len;j++)  s1 += '<option>' + choice[j].split('=')[0] + '</option>';
                           select.innerHTML = s1;
                       }
                        break;
                    case 4: //single text
